...
describe "GET /tasks" do
  ...
  context "when filter param is sent " do
    ...
    let!(:other_task_2){ create(:task, title: "Buy a the new car", user_id: user.id)}
    ...
    it "returns only the task matching" do
      returnd_task_titles = json_body[:data].map{ |t| t[:attributes][:title]}
      expect(returnd_task_titles).to eq([notebook_task_1.title, notebook_task_2.title])
    end
...
describe "POST /tasks" do
  ...
  context "when the params are valid" do
    ...
    it "assigns the created to the current user" do
      expect(json_body[:data][:attributes][:'user-id']).to eq(user.id)
    end
  end


  context "when the parems are invalid " do
